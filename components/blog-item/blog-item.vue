	...
	<view class="blogitem">
		<view class="head">
			<view class="userinfo">
				<view class="avatar">
					<image 
					:src="giveAvatar(item)"  mode="aspectFill"></image>
				</view>
				<view class="name">
					{{giveName(item)}}
				</view>
		...
		<view class="info">
			...
			<view @click="clickLike" class="box" :class="item.isLike ? 'active' : ''">
				<text class="iconfont icon-a-106-xihuan"></text> 
				<text>{{item.like_count ? item.like_count : '点赞'}}</text>
			</view>
		</view>
		<u-action-sheet :actions="list" cancelText="取消" 
		:show="sheetShow" :closeOnClickAction="true" :closeOnClickOverlay="true"
...
<script>
	...
	export default {
		...
		methods:{
			giveName,	//导入的方法在methods中执行一次才能在标签内拿到
			giveAvatar,
			// 单击图片
			clickPic(index){
				uni.previewImage({
			...
			clickMore(){
				...
				if(uid == this.item.user_id[0]._id || this.uniIDHasRole('admin') || this.uniIDHasRole('master')){
